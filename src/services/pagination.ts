...
export const PaginationService = class PaginationClass{
    public static async getCommitsInfo(response: Response): Promise<Array<CommitCardInterface>>{
        const responseJson = await response.json();
        const repositories = responseJson as Array<any>;
        const commits:Array<CommitCardInterface> = repositories.map((repository, index) => {
            return {
